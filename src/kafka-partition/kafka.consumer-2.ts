...
const group = 'tomyum-group-partition';
const consumer = getKafka('consumer-2').consumer({groupId: group });
  ...
  try {
    ...
    await consumer.run({
      eachMessage: async ({ topic, partition, message }) => {
        console.log(`[${group}][Consumer 2] Received message: ${message.value?.toString()} from partition ${partition}`);
      },
    });
